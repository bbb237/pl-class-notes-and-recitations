...
(define (can-floor-it l) (or (can-go l) (can-go (next-light l))))
...
(check-expect (next-light-iter 1 'yellow) 'red)
(check-expect (next-light-iter 27 'red) 'red)
...
(check-expect (safe-to-scroll 'sportscar 137 'green) #t)
; etc. 
...
(define (all-even-map l) (all (map is-even l)))


(check-expect (all-even-map '(2 4 6)) #t)
(check-expect (all-even-map '(2 4 7 4 6)) #f)
...
;; Write your solution here
 ...
 (define (reverse-helper l l_acc)
   (match l
     ['() l_acc]
     [(cons head tail) (reverse-helper tail (cons head l_acc))]))

 (define (reverse-list-opt l) (reverse-helper l '()))

(check-expect (reverse-list-opt '(2 4 6)) '(6 4 2))
(check-expect (reverse-list-opt '(#t 3)) '(3 #t))
(check-expect (reverse-list-opt '('(#t 3) "list")) '("list" '(#t 3)))
