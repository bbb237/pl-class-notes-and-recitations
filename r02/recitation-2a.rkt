...
(define (evaluate-bexp-m b)
  (match b
    ...
    [(not-node b1)    (not (evaluate-bexp-m b1))]))

; (check-expect (evaluate-bexp-m (and-node (leaf-node #t) (leaf-node #f))) #f)
...
(define
  ...
  (match b
    ...
    [(not-node b1) (match b1 ...)
    [(not-node (leaf-node v))   b]
    [(not-node (or-node b1 b2)) (and-node (not-node (demorgan-opt b1)) (not-node (demorgan-opt b2)))]
    [(not-node (and-node b1 b2)) (or-node (not-node (demorgan-opt b1)) (not-node (demorgan-opt b2)))]
    [(not-node (not-node b)) (demorgan-opt b)]))


;; 3. Testing our compiler optimization pass

;; A cursory test function: determine if two bexp are syntactically equal
(define
  (syn-equal? b1 b2)
  (match* (b1 b2)
    [((and-node b1 b2) (and-node b3 b4)) (and (syn-equal? b1 b3) (syn-equal? b2 b4))]
    ; ...
