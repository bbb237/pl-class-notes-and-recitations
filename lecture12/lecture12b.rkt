...
(define-type Expr
  ...
  [nondetC] ; when executing nondetC, we non-deterministically get either #t or #f
  )
...
(define (product-list (l1 : (listof 'a)) (l2 : (listof 'b))) :  (listof ('a * 'b))
  (mjoin (map (lambda (x) (map (lambda (y) (pair x y)) l2)) l1)))
...
(define (eval-env (env : Env) (e : Expr)) : (listof Value)
  (type-case Expr e
             ...
             [nondetC () (list (boolV #t) (boolV #f))]
             [plusC (e1 e2)
                    (let ([l1 (eval-env env e1)]
                          [l2 (eval-env env e2)])
                     (map (lambda (p) (numV (+ (numV-n (fst p)) (numV-n (snd p)))))
                                       (product-list l1 l2)))]
             [timesC (e1 e2)
                    (let ([l1 (eval-env env e1)]
                          [l2 (eval-env env e2)])
             ...
             [equal?C (e1 e2)
                      (let ([l1 (eval-env env e1)]
                            [l2 (eval-env env e2)])
                        (map (lambda (p) (boolV (equal? (fst p) (snd p))))
                                        (product-list l1 l2)))]
        #|     [ifC (guard e1 e2)
...
#|
(define e1 (parse '(let fact (rec fact x
                               (if (nondet)
                                   1
                                   (if (equal? x 0)
                                       1
