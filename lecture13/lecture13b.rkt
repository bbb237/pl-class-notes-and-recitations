...
(define-type-alias (M 'a) (listof (number * 'a)))

(define (scale (p : number) (ls : (M 'a))) : (M 'a)
  (map (lambda (x) (pair (* p (fst x)) (snd x))) ls))

(define (mjoin (ls : (M (M 'a)))) : (M 'a)
  (cond
    [(empty? ls) empty]
    [else
     (append (scale (fst (first ls)) (snd (first ls)))
             (mjoin (rest ls)))]))

(define (fmap (f : ('a -> 'b)) (a : (M ' a))) : (M 'b)
  (map (lambda (px) (pair (fst px) (f (snd px)))) a))

(define (mbind (ls : (M 'a)) (f : ('a -> (M 'b)))) : (M 'b)
  (mjoin (fmap f ls)))

(define (mreturn (x : 'a)) : (M 'a)
  (list (pair 1 x)))
...
(define (eval-env (env : Env) (e : Expr)) : (M Value)
  (type-case Expr e
             ...
             [flipC () (list (pair 1/2 (boolV #f)) (pair 1/2 (boolV #t)))]
...
(eval (parse '(+ (if (flip) 0 1) (if (flip) 0 1))))

(define (pr (prop : ('a -> boolean)) (outcomes : (M 'a))) : number
  (foldl + 0 (map fst (filter (lambda (px) (prop (snd px))) outcomes))))

(define res1
  (eval (parse '(let sum-coins
                  (rec sum-coins x
                          (if (equal? x 0)
                            0
                            (+ (if (flip) 0 1) (sum-coins (+ x -1)))))
                  (sum-coins 10)))))
