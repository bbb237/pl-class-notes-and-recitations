...
;; This file extends lecture6c.rkt by implementing mutable variables. We do so
;; by making the environment map variables to *locations* in the store, and
;; then do mutable updates to the corresponding locations in the store.
       ...
       (cond
         [(s-exp-symbol? (first l))
          (case (s-exp->symbol (first l))
            ...
            [(set!) (setC (s-exp->symbol (second l)) (parse (third l)))]
  ...
  (type-case Expr e
             ...
             [setC (x e)
                   (type-case Result (eval-env env sto e)
                              [res (v sto-1)
                                   (let [(l (lookup x env))]
                                     (res v (override-store (cell l v) sto-1)))])]
             [seqC (e1 e2)
                      (type-case Result (eval-env env sto e1)
                        [res (v1 sto-1)
...
(define ex6 (parse '(let v (+ 1 1) (begin (set! v 3) (+ v v)))))
